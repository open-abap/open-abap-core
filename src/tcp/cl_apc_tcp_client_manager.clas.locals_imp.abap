...
CLASS lcl_client IMPLEMENTATION.
  ...
  METHOD if_apc_wsp_client~connect.
    ...
    " servername required for SNI
    WRITE '@KERNEL this.client = connect.connect({ port: this.mv_port.get(), host: this.mv_host.get(), servername: this.mv_host.get()}, () => {this.mo_handler.get().if_apc_wsp_event_handler$on_open();});'.
